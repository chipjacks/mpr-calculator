module Main exposing (Model, Msg(..), distanceListItem, init, main, menuItem, stepItem, timeInput, trainingPaceListItem, update, updateLevel, view, viewEquivalentRaceTimes, viewLevel, viewTrainingPaces)
...
menuItem onClickMsg activatesValue modelValue textValue textDescription =
    a
        [ class <|
            "item"
                ++ (if modelValue == activatesValue then
                        " active"

                    else
                        ""
                   )
        , onClick (onClickMsg activatesValue)
        ]
...
viewEquivalentRaceTimes modelDistance level =
    let
        timesList =
            level |> Result.andThen equivalentRaceTimes
    in
    case timesList of
        Ok list ->
...
distanceListItem distance modelDistance timeStr =
    div
        [ class <|
            "item"
                ++ (if distance == modelDistance then
                        " active"
...
viewTrainingPaces level =
    let
        pacesList =
            level |> Result.andThen trainingPaces
    in
    case pacesList of
        Ok list ->
